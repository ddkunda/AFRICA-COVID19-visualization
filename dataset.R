...
Africa_countries <- read.csv("../AFRICA_COVID19_visualization/population.csv", header = TRUE)
...
translationContent <- read.csv("../AFRICA_COVID19_visualization/dictionary.csv", header = TRUE)
...
names(Global_data) <- c("Country", "TotalCases", "NewCases", "TotalDeaths", "NewDeaths", "TotalRecovered", 
                "ActiveCases", "SeriousCritical", "Tot Cases/1M pop", "Deaths/1M pop","TotalTests", "Tests/1M pop") 
...
set2 <- Global_data[Global_data$Country %in% AFRICA, c("Country" ,"TotalCases","TotalDeaths", "TotalRecovered")]


######################################################################################################################                
### merge set1 and set2 
set1_2 <- merge(set1, set2, by="Country", all = TRUE)
#set1_2$TotalCases[set1_2$Country %in% "Central African Republic"] <- set1_2$Cases[set1_2$Country %in% "Central African Republic"]

#set1_2 <- set2 

## update columns
set1_2$TotalCases <- as.character(set1_2$TotalCases)
set1_2$TotalCases <- gsub(",", "", set1_2$TotalCases)
set1_2$TotalCases <- as.integer(set1_2$TotalCases)    

set1_2$TotalDeaths <- as.character(set1_2$TotalDeaths)
set1_2$TotalDeaths <- gsub(",", "", set1_2$TotalDeaths)
set1_2$TotalDeaths <- as.integer(set1_2$TotalDeaths)    

set1_2$TotalRecovered <- as.character(set1_2$TotalRecovered)
set1_2$TotalRecovered <- gsub(",", "", set1_2$TotalRecovered)
set1_2$TotalRecovered <- as.integer(set1_2$TotalRecovered)    
...
CASES_DEATHS_RECOVERED$Region[CASES_DEATHS_RECOVERED$Country %in% c("Benin", "Burkina Faso","Cape Verde", "Côte d'Ivoire", "Gambia", "Ghana","Guinea", "Guinea-Bissau","Liberia","Mali","Niger", "Nigeria","Senegal", "Sierra Leone", "Togo")] <- "Western"


## save
write.csv(CASES_DEATHS_RECOVERED, "../AFRICA_COVID19_visualization/CASES_DEATHS_v1.csv", row.names = FALSE)

## load my own collected dataset
CASES_DEATHS_RECOVERED <- read.csv("../AFRICA_COVID19_visualization/CASES_DEATHS_v1.csv", header = TRUE)
CASES_DEATHS_RECOVERED2 <- read.csv("../AFRICA_COVID19_visualization/CASES_DEATHS_v2.csv", header = TRUE)

######################################################################################################################################################################################                
######################################################################################################################################################################################                

if(sum(CASES_DEATHS_RECOVERED$TotalCases, na.rm = TRUE) > sum(CASES_DEATHS_RECOVERED2$TotalCases, na.rm = TRUE)){
...
CASES_DEATHS_RECOVERED3$CasTotal <- CASES_DEATHS_RECOVERED3$TotalCases
CASES_DEATHS_RECOVERED3$DécèsTotal <- CASES_DEATHS_RECOVERED3$TotalDeaths
CASES_DEATHS_RECOVERED3$RétablisTotal <- CASES_DEATHS_RECOVERED3$TotalRecovered
...
}else if(sum(CASES_DEATHS_RECOVERED$Cases, na.rm = TRUE) < sum(CASES_DEATHS_RECOVERED2$Cases, na.rm = TRUE)){
...
CASES_DEATHS_RECOVERED3$Case.Rate <- round(((CASES_DEATHS_RECOVERED3$TotalCases/CASES_DEATHS_RECOVERED3$Pop.)*1000000),2)
CASES_DEATHS_RECOVERED3$Death.Rate <- round(((CASES_DEATHS_RECOVERED3$TotalDeaths/CASES_DEATHS_RECOVERED3$Pop.)*1000000),2)

CASES_DEATHS_RECOVERED3$Tx.Cas <- CASES_DEATHS_RECOVERED3$Case.Rate
CASES_DEATHS_RECOVERED3$Tx.Mortalité <- CASES_DEATHS_RECOVERED3$Death.Rate

save(CASES_DEATHS_RECOVERED3, file = "../AFRICA_COVID19_visualization/CASES_DEATHS_v3.RData")
...
#save(CASES_DEATHS_long2, file="../AFRICA_COVID19_visualization/coronavirus_continent.RData")
